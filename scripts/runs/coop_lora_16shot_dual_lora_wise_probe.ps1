...
$OutputRoot = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_16shot_dual_lora_wise_probe"
$SummaryCsv = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_16shot_dual_lora_wise_probe_summary.csv"
$SourceTrainDir = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_16shot_text_lora_probe\models\dual_lora_l8\train"
...
$alphas = @(0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0)

if (-not (Test-Path -LiteralPath $SourceCkpt)) {
    throw "Missing source dual-LoRA 16-shot checkpoint: $SourceCkpt"
}

if (-not $KeepExisting) {
    if (Test-Path -LiteralPath $OutputRoot) {
        "Removing existing 16-shot dual-LoRA WiSE output: $OutputRoot"
        Remove-Item -LiteralPath $OutputRoot -Recurse -Force
    }
    if (Test-Path -LiteralPath $SummaryCsv) {
        "Removing existing 16-shot dual-LoRA WiSE summary: $SummaryCsv"
...
function Invoke-CoOpLoRAEval {
    ...
    python -u "$CoOpRoot\train.py" `
        ...
        TRAINER.COOPLORA.LAYERS 8 `
        TRAINER.COOPLORA.TEXT_LAYERS 8 `
