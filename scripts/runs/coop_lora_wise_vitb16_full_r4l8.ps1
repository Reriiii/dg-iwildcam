...
$OutputRoot = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_10ep_r4_l8_wise"
$SummaryCsv = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_10ep_r4_l8_wise_summary.csv"
...
$SourceCkpt = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_10ep_ablation\r4_l8\train\coop_lora\model.pth.tar-10"
...
if (-not (Test-Path -LiteralPath $SourceCkpt)) {
    throw "Missing source checkpoint: $SourceCkpt"
}
...
foreach ($alpha in $alphas) {
    ...
    Invoke-CoOpLoRAEval -EvalName "id_val_$Name" -ModelDir $ModelDir -TestSplit "val" -IWildCamTestSplit "val"
    Invoke-CoOpLoRAEval -EvalName "ood_val_$Name" -ModelDir $ModelDir -TestSplit "test" -IWildCamTestSplit "val"
}
...
if (-not $BestId) {
    throw "No id_val rows found in $SummaryCsv"
}
if (-not $BestOod) {
    throw "No ood_val rows found in $SummaryCsv"
}
...
$SelectedCases = @($BestIdCase, $BestOodCase) | Select-Object -Unique
foreach ($case in $SelectedCases) {
    $ModelDir = Join-Path $OutputRoot ("models\" + $case + "\train")
    Invoke-CoOpLoRAEval -EvalName "ood_test_$case" -ModelDir $ModelDir -TestSplit "test" -IWildCamTestSplit "test"
}
