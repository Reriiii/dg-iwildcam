...
$OutputRoot = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_10ep_r4_l8_supcon0p5_wise"
$SummaryCsv = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_10ep_r4_l8_supcon0p5_wise_summary.csv"
...
if (-not $KeepExisting) {
    if (Test-Path -LiteralPath $OutputRoot) {
        "Removing existing output for clean SupCon-WiSE run: $OutputRoot"
        Remove-Item -LiteralPath $OutputRoot -Recurse -Force
    }
    if (Test-Path -LiteralPath $SummaryCsv) {
        "Removing existing summary for clean SupCon-WiSE run: $SummaryCsv"
...
function Invoke-CoOpLoRAEval {
    ...
    python -u "$CoOpRoot\train.py" `
        ...
        TRAINER.COOPLORA.SUPCON_WEIGHT 0.0 `
...
if (-not (Test-Path -LiteralPath $SourceCkpt)) {
    python -u "$CoOpRoot\train.py" `
        ...
        DATALOADER.TEST.BATCH_SIZE 512 `
        TRAINER.COOPLORA.SUPCON_WEIGHT $supconWeight `
        TRAINER.COOPLORA.SUPCON_TEMPERATURE $supconTemperature `
        TRAINER.COOPLORA.L2_INIT_WEIGHT 0.0
} else {
