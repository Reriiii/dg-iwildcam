...
$OutputRoot = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_class_domain_cvar_pilot"
$DetailCsv = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_class_domain_cvar_pilot_summary.csv"
...
$maxEpoch = 3
$variants = @(
    @{Name = "ce_3ep_random"; Weight = 0.0; Rho = 0.2},
    @{Name = "ccloccvar0p1_rho0p2"; Weight = 0.1; Rho = 0.2},
    @{Name = "ccloccvar0p2_rho0p2"; Weight = 0.2; Rho = 0.2},
    @{Name = "ccloccvar0p3_rho0p3"; Weight = 0.3; Rho = 0.3}
)
$splits = @(
...
if (-not $KeepExisting) {
    if (Test-Path -LiteralPath $OutputRoot) {
        "Removing existing class-domain-CVaR pilot output: $OutputRoot"
        Remove-Item -LiteralPath $OutputRoot -Recurse -Force
    }
    if (Test-Path -LiteralPath $DetailCsv) {
        "Removing existing class-domain-CVaR pilot summary: $DetailCsv"
...
function Invoke-TrainVariant {
    ...
    $lossOpts = @()
    if ([double]$Variant.Weight -gt 0) {
        $lossOpts = @(
            "TRAINER.COOPLORA.CLASS_DOMAIN_CVAR_WEIGHT", ("{0:0.####}" -f [double]$Variant.Weight),
            "TRAINER.COOPLORA.CLASS_DOMAIN_CVAR_RHO", ("{0:0.####}" -f [double]$Variant.Rho)
        )
    }
