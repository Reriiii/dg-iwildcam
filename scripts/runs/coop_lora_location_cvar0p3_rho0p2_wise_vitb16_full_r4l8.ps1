...
$OutputRoot = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_10ep_r4_l8_location_cvar0p3_rho0p2_wise"
$SummaryCsv = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_10ep_r4_l8_location_cvar0p3_rho0p2_wise_summary.csv"
...
if (-not $KeepExisting) {
    if (Test-Path -LiteralPath $OutputRoot) {
        "Removing existing output for clean Location-CVaR-WiSE run: $OutputRoot"
        Remove-Item -LiteralPath $OutputRoot -Recurse -Force
    }
    if (Test-Path -LiteralPath $SummaryCsv) {
        "Removing existing summary for clean Location-CVaR-WiSE run: $SummaryCsv"
...
if (-not (Test-Path -LiteralPath $SourceCkpt)) {
    python -u "$CoOpRoot\train.py" `
        ...
        DATALOADER.TEST.BATCH_SIZE 512 `
        TRAINER.COOPLORA.DOMAIN_CVAR_WEIGHT $cvarWeight `
        TRAINER.COOPLORA.DOMAIN_CVAR_RHO $cvarRho
} else {
