...
$TrainerConfig = Join-Path $CoOpRoot "configs\trainers\CoOpLoRA\vit_b16_iwildcam_full_20ep_r4_l8.yaml"
$OutputRoot = Join-Path $RepoRoot "outputs\coop_lora_iwildcam_vitb16_full_20ep_r4_l8_clean"
...
if (-not (Test-Path -LiteralPath $FinalCkpt)) {
    ...
    "Skipping clean train because checkpoint exists: $FinalCkpt"
}

foreach ($epoch in $epochs) {
    $Ckpt = Join-Path $TrainDir "coop_lora\model.pth.tar-$epoch"
